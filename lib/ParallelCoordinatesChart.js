...
function ParallelCoordinatesChart(parent, pathToCSV, filter, callback) {
	...
	d3.csv(this.pathToCSV, function(error, results) {
		...
		self.dimensions = d3.keys(results[0]).filter(function(d) {
			return filter ? !self.filter.includes(d) : true;
		});

		self.results.forEach(function(item, index) {
		...
		self.axes.append("g")
			...
			.each(function(d) {
				if (!d.startsWith("output_")) {
					...
					d3.select(this).call(self.outputAxis.scale(self.y[d]));
				}
			})
		.append("text")
